# Prepare the longitudinal, survival, and practice-effect analysis datasets.
#
# Inputs: controlled LASI/LASI-DAD files configured through environment
# variables, plus dad_tracker_v1.dta, H_LASI_DAD_b1.dta, and variable_map.xlsx.
# Output: data/derived/processed_data.rds.
...
paths <- project_paths()
lasi_raw_dir <- required_environment_directory("LASI_RAW_DIR")
harmonized_dir <- required_environment_directory("HARMONIZED_DATA_DIR")
exit_dir <- required_environment_directory("LASIDAD_EXIT_DIR")
...
lasi_data <- read_dta(required_input_file(file.path(harmonized_dir, "H_LASI_a3.dta")))
...
rawlasi_dt <- as.data.table(read_dta(required_input_file(file.path(lasi_raw_dir, "lasi_w1b_ind_bm.dta"))))

lasidad_dt <- as.data.table(read_dta(required_input_file(file.path(harmonized_dir, "h_dad_w1a3.dta"))))

exit_dt <- as.data.table(read_dta(required_input_file(file.path(exit_dir, "dad_exit_clean.dta"))))

attrition_dt <- as.data.table(read_dta(required_input_file(file.path(rawdata_dir, "dad_tracker_v1.dta"))))

longitudinal_dt <- as.data.table(read_dta(required_input_file(file.path(rawdata_dir, "H_LASI_DAD_b1.dta"))))

varmap_dt <- as.data.table(read.xlsx(required_input_file(file.path(rawdata_dir, "variable_map.xlsx"))))
...
# CLASSIFY THOSE WHO REFUSED OR WERE SICK AS CENSORED AT VISIT DATE --------
...
write_rds(
  list(survival = survival_dt, longitudinal = long_dt, practice = pe_dt),
  file.path(derived_dir, "processed_data.rds")
)
