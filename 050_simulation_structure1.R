# Run simulation 1 for the regression-to-the-mean sensitivity analysis.
#
# Input: data/derived/processed_data_weights.rds from 011_weights.R.
# Outputs: data/derived/simulation1.xlsx and simulation1.rds for script 052.
...
sim_data <- function(sample_size = 2000, w2_effect){
    ...
    #model_sim <- lmer(gcp ~ time * any_school + (1 | prim_key), data = lsim_dt) save time by modeling using linear regression since they are the same at the mean 
    model_sim <- lm(gcp ~ time * any_school, data = lsim_dt)
...
for (effect in c(w2cog_params[Parameter == "w1_cog", Coefficient])){
...
sim_summary <- full_sim_results[, .(effect = "Estimate", mean = mean(int_value), lower = quantile(int_value, 0.025), upper = quantile(int_value, 0.975))]
...
write.xlsx(sim_summary, file.path(derived_dir, "simulation1.xlsx"))
write_rds(sim_plot, file.path(derived_dir, "simulation1.rds"))
