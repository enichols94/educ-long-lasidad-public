# Create the descriptive table for the weighted analysis sample.
#
# Input: data/derived/processed_data_weights.rds from 011_weights.R.
# Output: a date-stamped spreadsheet in the table output directory.
...
table_dir <- paths$table1
ensure_output_directory(table_dir)
...
get_col <- function(educ_cat){
    ...
    cog <- get_continuous_raw("gcp", num = 2)
    ...
    col <- data.table(name = c(N, "", outcome, followup, cog, age, female, rural, "", caste, "", educ_dad,
...
labels <- c("N", "Wave 2 status", paste0("  ", dt[, levels(outcome)]), "Follow-up time", "Cognition (SD units)", "Age (yrs)", "Female (%)", "Rural (%)", "Caste", 
...
write.xlsx(table_dt, file.path(table_dir, paste0("table1_", date, ".xlsx")))
