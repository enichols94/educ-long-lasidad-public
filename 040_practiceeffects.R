# Estimate practice effects overall and by education group.
#
# Inputs: processed_practiceeffects_data.rds from 012_pe_weights.R,
# H_DAD_w12_noimp_core.dta, and practiceeffects_variable_map.xlsx.
# Output: a date-stamped practice-effects PDF.
...
plot_dir <- paths$practice_figures
ensure_output_directory(plot_dir)

# READ DATA ------------------------------------------------------------------

pe_dt <- read_rds(required_input_file(file.path(derived_dir, "processed_practiceeffects_data.rds")))

cog_dt <- as.data.table(read_dta(required_input_file(file.path(rawdata_dir, "H_DAD_w12_noimp_core.dta"))))

pe_map <- as.data.table(read.xlsx(required_input_file(file.path(rawdata_dir, "practiceeffects_variable_map.xlsx"))))
...
ggsave(file.path(plot_dir, paste0("practice_effects_", date, ".pdf")), plot = full_graph, width = 10, height = 7)
