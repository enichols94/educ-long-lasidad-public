# Estimate inverse-probability weights for the practice-effect analysis.
#
# Input: data/derived/processed_data.rds from 010_dataprep.R.
# Outputs: processed_practiceeffects_data.rds and balance diagnostics.
...
load_required_packages(c(
  ...
  "lubridate", "survival", "patchwork", "srvyr", "pROC", "Hmisc",
  "weights", "parameters"
))
...
plot_dir <- paths$appendix
ensure_output_directory(plot_dir)

# READ DATA ------------------------------------------------------------------

data <- read_rds(required_input_file(file.path(derived_dir, "processed_data.rds")))
pe_dt <- data$practice
...
# MODEL EXPOSURE TO PRACTICE (REFRESHER VERSUS RETURNER SAMPLE) ------------
...
## restrict to the trimmed dataset
...
pdf(file.path(plot_dir, paste0("practiceeffects_balance_plots_", date, ".pdf")), height = 8, width = 14)
pe_balance; pe_restricted_balance
dev.off()

pdf(file.path(plot_dir, paste0("practiceeffects_density_plots_", date, ".pdf")), height = 4, width = 4)
...
write_rds(pe_restricted_dt,
          file.path(derived_dir, "processed_practiceeffects_data.rds"))
