# Estimate mortality and attrition weights for the longitudinal analyses.
#
# Input: data/derived/processed_data.rds from 010_dataprep.R.
# Outputs: processed_data_weights.rds and balance diagnostic figures.
...
pdf(file.path(plot_dir, paste0("balance_plots_", date, ".pdf")), height = 8, width = 8)
death_balance; attrition_balance; combo_balance
dev.off()

ggsave(file.path(appendix_dir, paste0("mortality_balance_plot_", date, ".pdf")), plot = death_balance, width = 8, height = 8)
...
longitudinal_dt[wave == 1, c("death_weight", "attrition_weight", "combo_weight") := 1]
longitudinal_dt[wave == 1, c("full_weight") := weight_w1] 

write_rds(list(survival = survival_dt, longitudinal = longitudinal_dt),
          file.path(derived_dir, "processed_data_weights.rds"))
